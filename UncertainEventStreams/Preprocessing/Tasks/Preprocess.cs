...
FROM DS A
WHERE [Stop ID] NOT IN(SELECT TOP 1 [Stop ID] FROM JourneyPatterns WHERE [Journey Pattern ID]=A.[Journey Pattern ID] ORDER BY [Stop Index] DESC)
...
UNION ALL 
SELECT @VehicleJourneyId AS [Vehicle Journey ID], A.[Journey Pattern ID],  A.[Stop ID], NULL AS Timestamp, '2-Resume' AS [EventType],[Station Log Index] = 1
FROM JourneyPatterns A 
WHERE A.[Journey Pattern ID]=@JourneyPatternId AND A.[Stop ID] NOT IN (
SELECT DISTINCT [Stop ID]
FROM EventLog
WHERE [Journey Pattern ID]=@JourneyPatternId AND  [Vehicle Journey ID]=@VehicleJourneyId)
UNION ALL
SELECT @VehicleJourneyId AS [Vehicle Journey ID], A.[Journey Pattern ID],  A.[Stop ID], NULL AS Timestamp, '4-Suspend' AS [EventType],[Station Log Index] = 2
FROM JourneyPatterns A 
WHERE A.[Journey Pattern ID]=@JourneyPatternId AND A.[Stop ID] NOT IN (
SELECT DISTINCT [Stop ID]
FROM EventLog
...
FROM EX A LEFT JOIN JourneyPatterns B ON A.[Journey Pattern ID]=B.[Journey Pattern ID] AND A.[Stop ID] = B.[Stop ID]
